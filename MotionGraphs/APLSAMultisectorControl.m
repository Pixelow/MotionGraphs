...
} APLSASectorDrawingInformation;


@implementation APLSAMultisectorControl{
    NSMutableArray *sectorsArray;
    
    APLSAMultisectorSector *trackingSector;
    APLSASectorDrawingInformation trackingSectorDrawInf;
...
- (BOOL) beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event{
    ...
    for(NSUInteger i = 0; i < sectorsArray.count; i++){
        APLSAMultisectorSector *sector = sectorsArray[i];
        NSUInteger position = i + 1;
        APLSASectorDrawingInformation drawInf =[self sectorToDrawInf:sector position:position];
...
- (void)drawSector:(APLSAMultisectorSector *)sector atPosition:(NSUInteger)position{
...
- (APLSASectorDrawingInformation) sectorToDrawInf:(APLSAMultisectorSector *)sector position:(NSInteger)position{
    APLSASectorDrawingInformation drawInf;
...
@implementation APLSAMultisectorSector
