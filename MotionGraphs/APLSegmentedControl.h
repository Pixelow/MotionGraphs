//
//  APLSegmentedControl.h
//  APLSegmentedControl
//
//  Created by M on 22/5/17.
//

#import <UIKit/UIKit.h>

@protocol APLSegmentedControlDelegate;

/**
 A drop-in replacement for UISegmentedControl showing multiple segment counts, to be used typically on a user profile.
 */
@interface APLSegmentedControl : UIControl <UIBarPositioning, UIAppearance>

/** The control's delegate object, conforming to the UIBarPositioning protocol. */
@property (nonatomic, weak) id <APLSegmentedControlDelegate> delegate;
...
/**
 ...
 @returns A APLSegmentedControl object or nil if there was a problem in initializing the object.
 */
...
/**
 The APLSegmentedControlDelegate protocol defines the interface that APLSegmentedControl delegate objects implement to manage the segmented control behavior. This protocol declares no methods of its own but conforms to the UIBarPositioningDelegate protocol to support the positioning of a segmented control when it is moved to a window.
 */
@protocol APLSegmentedControlDelegate <UIBarPositioningDelegate>
