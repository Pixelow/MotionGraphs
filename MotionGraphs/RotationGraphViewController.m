...
@interface RotationGraphViewController ()
...
@implementation RotationGraphViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    //set the title
    self.title = @"Rotation Time History";
    ...
    [self drawTimeHistoryPlot:self.gyroArrayX];
}

- (IBAction)plotAlongDeviceXAxis:(UIButton *)sender
{
...
- (IBAction)plotAlongDeviceYAxis:(UIButton *)sender
{
    [graph removeFromSuperlayer];
    [self drawTimeHistoryPlot:self.gyroArrayY];
...
- (IBAction)plotAlongDeviceZAxis:(UIButton *)sender
{
    [graph removeFromSuperlayer];
    [self drawTimeHistoryPlot:self.gyroArrayZ];
...
//    graph.plotAreaFrame.masksToBorder   = YES;
    ...
    for(int i=0;i<gyroArray.count;i++) {
        NSNumber* number = gyroArray[i];
    ...
    plotSpace.xRange = [CPTPlotRange plotRangeWithLocation:[NSNumber numberWithInt:0] length:[NSNumber numberWithFloat:(gyroArray.count/self.samplingRate)]];
    ...
    x.majorIntervalLength         = [NSNumber numberWithFloat:gyroArray.count/self.samplingRate/5.0]; // 5 being the grid number
    x.orthogonalPosition          = [NSNumber numberWithFloat:min];
    ...
    y.preferredNumberOfMajorTicks = 5.0f;
...
-(NSUInteger)numberOfRecordsForPlot:(CPTPlot *)plotnumberOfRecords {
    ...
    if ( [identifier isEqualToString:@"Gyro Data"] ) {
...
- (void)setAxesTitle
{
    ...
    yAxesLabel.text = NSLocalizedString(@"angle (degrees)","");
...
- (void)setXYZButton
{
    ...
    [self.xDataButton02 addTarget:self action:@selector(plotAlongDeviceXAxis:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self.yDataButton02 addTarget:self action:@selector(plotAlongDeviceYAxis:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self.zDataButton02 addTarget:self action:@selector(plotAlongDeviceZAxis:) forControlEvents:UIControlEventTouchUpInside];
