...
NSString *const APLElementView = @"APLElementView";
NSString *const APLElementKeyPath = @"APLElementKeyPath";
NSString *const APLElementFromValue = @"APLElementFromValue";
NSString *const APLElementToValue = @"APLElementToValue";

#define kDURATION   0.35

@interface APLViralSwitch ()
...
- (void)animateElementsFrom:(NSArray *)elements {
    for (NSDictionary *element in elements) {
        if ([element[APLElementKeyPath] isEqualToString:@"textColor"] && [element[APLElementView] isKindOfClass:[UILabel class]]) {
            [UIView transitionWithView:element[APLElementView]
                              duration:kDURATION
                               options:UIViewAnimationOptionTransitionCrossDissolve
                            animations:^{
                                [(UILabel *)element[APLElementView] setTextColor:element[APLElementToValue]];
                            }
                            completion:nil];
        } else if ([element[APLElementKeyPath] isEqualToString:@"tintColor"] && [element[APLElementView] isKindOfClass:[UIButton class]]) {
            [UIView transitionWithView:element[APLElementView]
                              duration:kDURATION
                               options:UIViewAnimationOptionTransitionNone
                            animations:^{
                                [(UIButton *)element[APLElementView] setTintColor:element[APLElementToValue]];
                            }
                            completion:nil];
        } else if ([element[APLElementKeyPath] isEqualToString:@"bgColor"] && [element[APLElementView] isKindOfClass:[UIButton class]]) {
            [UIView transitionWithView:element[APLElementView]
                              duration:kDURATION
                               options:UIViewAnimationOptionTransitionNone
                            animations:^{
                                [(UIButton *)element[APLElementView] setBackgroundColor:element[APLElementToValue]];
                            }
                            completion:nil];
        } else if ([element[APLElementKeyPath] isEqualToString:@"titleColor"] && [element[APLElementView] isKindOfClass:[UIButton class]]) {
            [UIView transitionWithView:element[APLElementView]
                              duration:kDURATION
                               options:UIViewAnimationOptionTransitionNone
                            animations:^{
                                [(UIButton *)element[APLElementView] setTitleColor:element[APLElementToValue] forState:UIControlStateNormal];
                            }
                            completion:nil];
        } else {
            CABasicAnimation *elementAnimation = [self animateKeyPath:element[APLElementKeyPath]
                                                            fromValue:element[APLElementFromValue]
                                                              toValue:element[APLElementToValue]
                                                               timing:kCAMediaTimingFunctionEaseIn];
            [element[APLElementView] addAnimation:elementAnimation forKey:element[APLElementKeyPath]];
        }
    }
}
